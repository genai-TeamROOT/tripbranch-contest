...
이전 추천 이력 없음(= current_conditions 없음)
  → 라우터 오분류로 본다. 되묻지 않고 RECOMMEND 추출로 구제한다
  → 2026-09-20 변경. 예전에는 "아직 추천한 결과가 없어요. 어떤 장소를
     찾고 계신가요?"로 단락했는데, "강남역 근처 놀만한곳 추천"처럼 이미
     구체적인 발화에 같은 질문을 되돌려주는 막다른 길이었다

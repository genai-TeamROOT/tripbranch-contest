...
- 실행 ID: `2026-08-27_1158_dev_current_35cases_99825c9642fa`
- 실행 시각: 2026-08-27T12:06:29+09:00
- 프롬프트 기준선: `current`
- 평가셋: `dev` · 35건 / 50턴
- 골드셋 해시: `99825c9642fa`
...
| 지표 | 결과 | 의미 |
| --- | ---: | --- |
| Intent Accuracy | 92.0% | 전체 턴에서 Intent가 일치한 비율 |
| Intent Macro F1 | 0.980 | Intent별 F1을 동등하게 평균낸 균형 점수 |
...
| 지표 | 결과 |
| --- | ---: |
| 클라이언트 지연시간 p50 | 5.69초 |
| 클라이언트 지연시간 p95 | 47.51초 |
...
| MODIFY | 11 | 100.0% | 81.8% | 0.900 |
| OUT_OF_SCOPE | 1 | 100.0% | 100.0% | 1.000 |
| RECOMMEND | 26 | 100.0% | 92.3% | 0.960 |
...
| INFO | 0 | 0 | 4 | 0 | 0 | 0 | 0 | 0 |
| MODIFY | 0 | 0 | 0 | 9 | 0 | 0 | 0 | 2 |
| OUT_OF_SCOPE | 0 | 0 | 0 | 0 | 1 | 0 | 0 | 0 |
| RECOMMEND | 0 | 0 | 0 | 0 | 0 | 24 | 0 | 2 |
| SCHEDULE | 0 | 0 | 0 | 0 | 0 | 0 | 6 | 0 |
...
- 기대 Intent: `RECOMMEND, MODIFY`
- 실제 Intent: `__ERROR__`
- 조건 불일치: `search_center` 기대 `'경복궁'` / 실제 `None`, `weather` 기대 `'rain'` / 실제 `None`, `weather_intent` 기대 `'AVOID'` / 실제 `None`, `environment` 기대 `'indoor'` / 실제 `None`
- 오류: [Errno 54] Connection reset by peer

### DEV-035 — 태그 교체

- 기대 Intent: `RECOMMEND, MODIFY`
- 실제 Intent: `__ERROR__`
- 조건 불일치: `search_center` 기대 `'광화문'` / 실제 `None`, `place_types` 기대 `['cultural_facility']` / 실제 `None`, `place_tags` 기대 `['미술관']` / 실제 `None`
- 오류: [Errno 61] Connection refused

## 직전 동일 골드셋 대비

비교 대상: `2026-08-27_1136_dev_current_35cases_99825c9642fa`

- intent_accuracy: -0.0800
- macro_f1: -0.0200

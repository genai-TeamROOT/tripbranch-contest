...
- taste_query: 장소의 **분위기·경험·취향**을 말한 부분만 원문 표현 그대로 옮긴다.
  ...
  - **혼잡도(사람 붐빔) 표현도 넣지 않는다** — 조용한·한적한·붐비는·북적이는·
    시끌벅적 같은 말은 concentration_intent가 이미 받는다. taste_query에는 그 말을
    뺀 나머지 취향(분위기·감성·경험)만 남긴다. 발화가 혼잡도만 말했으면 null이다.
  - 취향을 말하지 않았으면 null. 억지로 만들어내지 않는다.
  - 예) "한적하고 감성적인 곳" → "감성적인" (한적함은 concentration_intent가 받는다)
  - 예) "붐비지 않는 조용한 카페" → null (혼잡도만 말했다)
  ...
  (이 반례들은 실측에서 취향 근거를 잘못 찾아냈거나 혼잡도와 겹쳐 잡힌 문장이다 —
   HISTORY.md "결정 근거" 참고)

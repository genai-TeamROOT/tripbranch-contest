...
JSON의 items에 있는 사실만 사용해 비교 설명을 작성하세요. criteria=travel_time이면
1~2줄로, 그 외에는 3~6줄로 작성하세요.
...
- criteria가 time이면 남은 운영시간 차이를 우선 설명하고, 첫 문장에서 가장 알맞은
  한 곳을 분명히 추천하세요.
- criteria가 travel_time이면 `travel_distance_km`·`travel_walking_minutes`·
  `travel_driving_minutes`·`travel_transit_minutes` 같은 수치를 본문에 나열하지
  마세요 — 장소별 정확한 거리·수단별 소요시간은 답변 아래 카드에 따로 표시됩니다.
  각 장소의 수단별 소요시간 중 가장 짧은 값을 기준으로 비교해, 첫 문장에서 전반적으로
  가장 빨리·가깝게 갈 수 있는 곳을 분명히 추천하고, 필요하면 짧은 이유 한 문장만
  덧붙이세요. 도로 정체(실시간 교통 상황)는 아직 반영하지 못한다는 점을 답변에서

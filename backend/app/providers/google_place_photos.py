...
역할: 장소명·주소·좌표로 Google Places를 찾아 대표 사진 1장의 표시용 주소를 준다.
입력: 장소명(필수)과 주소·좌표(있으면 정확도를 올린다).
출력: 표시용 이미지 URL 1개. 찾지 못했거나 사진이 없으면 None.
...
1. Text Search로 장소를 찾으면서 사진 메타데이터까지 같이 받는다. 우리 DB에
   `google_place_id`가 없어 검색부터 해야 한다 — 예전에 적재했던
...
Google Places 정책상 사진 파일과 사진 이름(photo resource name)을 영구 저장할 수
없고, 표시용 주소는 만료된다. 그래서 `hybrid_place_photos.py`와 같은 방식으로
프로세스 메모리에만 들고 있다가 TTL이 지나면 다시 받는다. 캐시와 래치가 모듈
수준인 이유도 같다 — 이 Provider는 요청마다 새로 만들어지므로 인스턴스에 담으면
요청이 끝날 때 함께 사라진다.
"""
...
# Text Search가 돌려줄 필드. 필드를 넓게 잡을수록 비싼 SKU로 과금되므로 사진
# 이름 하나만 받는다. 이 헤더가 없으면 요청 자체가 400으로 거절된다.
_SEARCH_FIELD_MASK = "places.photos.name"
...
# 캐시 키 → (만료 시각(monotonic), 이미지 주소). None은 "Google에도 사진이
# 없다"는 확인된 사실이며 그것도 캐시한다 — 그러지 않으면 사진 없는 장소가
# 카드에 뜰 때마다 2회씩 계속 호출된다.
...
class FakeGooglePlacePhotoProvider:
    ...
    Fake가 항상 주소를 주면 그 분기가 한 번도 실행되지 않은 채 테스트가 통과한다.
    """

    # 이름에 이 문자열이 들어간 장소는 사진이 없는 것으로 답한다.
    NO_PHOTO_MARKER = "사진없음"

    def __init__(self, photo_url: str = "https://example.test/google-place-photo.jpg") -> None:
...
class GooglePlacePhotoProvider:
    """Google Places API(New)로 대표 사진 1장의 표시용 주소를 찾는다."""
    ...
    async def find_cover_photo(
        ...
        *,
        ...
        """장소 한 곳의 대표 사진 주소를 찾는다. 없으면 None.
        ...
        try:
            photo_name = await self._find_photo_name(
                query=query, latitude=latitude, longitude=longitude
            )
            if photo_name is None:
                # "Google에도 사진이 없다"는 확인된 사실이라 캐시한다.
        ...
        self._remember(cache_key, photo_url)
        return photo_url

    async def _find_photo_name(
        self,
        *,
        query: str,
        latitude: float | None,
    ...
    def _cached(self, key: str) -> tuple[str | None] | None:
        entry = _photo_cache.get(key)
        if entry is None:
            return None
        expires_at, photo_url = entry
    ...
    def _remember(self, key: str, photo_url: str | None) -> None:
        if self._cache_ttl_seconds <= 0:
            return
        if len(_photo_cache) >= _CACHE_PURGE_THRESHOLD:
            self._purge_expired()
        _photo_cache[key] = (time.monotonic() + self._cache_ttl_seconds, photo_url)
